/* ========================================
   BLUVIA Admin — API Integration (v2 Patched)
   ======================================== */
...
function cleanTitle(slug) {
  if (!slug) return '';
  return slug.replace(/^-+/, '').replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())
    .replace(/\s*\(\d{4}\)$/, '').trim();
}
...
async function loadWatchChart() {
  ...
  // Dynamic labels from API data
  ['watch-chart-labels', 'analytics-chart-labels'].forEach(id => {
...
async function loadUsers() {
  ...
  data.users.forEach(u => {
    ...
    // Derive status from last_active (API has no status field)
    const isActive = u.last_active != null;
    const statusClass = isActive ? 'active' : 'inactive';
    tbody.innerHTML += `
      <tr>
        <td data-label="User"><div class="table-user">
        ...
        <td data-label="Joined" style="color:var(--text-muted);font-size:var(--fs-xs);">${formatDate(u.created_at)}</td>
        <td data-label="Watches"><strong>${u.total_watches}</strong></td>
        <td data-label="Last Active" style="color:var(--text-muted);font-size:var(--fs-xs);">${timeAgo(u.last_active)}</td>
        <td data-label="Status"><span class="status-badge ${statusClass}">${isActive ? 'Active' : 'Inactive'}</span></td>
...
// ── Load Activity Feed (FIXED: use watch-history instead of broken /activity) ──
...
// ── Request Approve/Reject (FIXED: added onclick handlers) ──
async function approveRequest(id) {
  const result = await apiRequest(`/api/admin/requests/${id}`, 'PUT', { status: 'added', admin_note: 'Approved by admin' });
  if (result !== null) loadRequests();
}

async function rejectRequest(id) {
  const result = await apiRequest(`/api/admin/requests/${id}`, 'PUT', { status: 'rejected', admin_note: 'Rejected by admin' });
  if (result !== null) loadRequests();
}
...
async function loadRequests() {
  ...
  all.forEach(r => {
    ...
    const actions = r.status === 'pending'
      ? `<div style="display:flex;gap:4px;"><button class="action-btn" title="Approve" style="color:#22c55e;" onclick="approveRequest(${r.id})"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg></button><button class="action-btn danger" title="Reject" onclick="rejectRequest(${r.id})"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div>`
      : `<span style="font-size:var(--fs-xs);color:var(--text-muted);">Done</span>`;

    tbody.innerHTML += `
      <tr>
        <td data-label="User"><div class="table-user"><div class="avatar" style="width:28px;height:28px;background:linear-gradient(135deg,${color},${color}dd);font-size:0.6rem;">${initial}</div><span style="font-size:var(--fs-sm);">${r.user_name}</span></div></td>
...
// ── Load Catalog (FIXED: trim leading dashes, show "No eps") ──
async function loadCatalog() {
  ...
  data.dramas.forEach(d => {
    ...
    const eps = d.total_episodes > 0 ? d.total_episodes : '<span style="color:var(--text-muted);font-style:italic;">No eps</span>';
    tbody.innerHTML += `
      <tr>
